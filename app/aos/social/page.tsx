...
import type { ContentSignal, SocialPost } from '@/lib/aos/social/types'
      ...
      <div style={styles.containerStyle}>
        ...
        <section style={styles.cartBoxStyle}>
          ...
          <p style={styles.bodyStyle}>
            {snapshot.totalEngagement.toLocaleString()} engagement signals
            detected across the current sample dataset.
          </p>
        </section>
          ...
          {snapshot.strongestPost ? (
            <>
              ...
              <p style={styles.bodyStyle}>
                ...
                <br />
                Pillar: {snapshot.strongestPost.pillar ?? 'uncategorized'}
                <br />
                Format:{' '}
                {(snapshot.strongestPost.format ?? 'unknown').replace('_', ' ')}
                <br />
        ...
        <section style={styles.cartBoxStyle}>
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            {snapshot.strongestSignals.map((signal: ContentSignal) => (
              <div key={`${signal.postId}-${signal.signalType}`} style={styles.innerCardStyle}>
                <h2 style={styles.h2Style}>
        ...
        <section style={styles.cartBoxStyle}>
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            {sampleSocialPosts.map((post: SocialPost) => (
              <div key={post.id} style={styles.innerCardStyle}>
                <h2 style={styles.h2Style}>{post.hook ?? post.id}</h2>

                <p style={styles.bodyStyle}>
                  {post.platform} · {post.pillar ?? 'uncategorized'} ·{' '}
                  {(post.format ?? 'unknown').replace('_', ' ')}
                </p>
        ...
        <Link href="/aos" style={styles.quietLinkStyle}>
          ← Back to AOS
        </Link>
      </div>
    </main>
