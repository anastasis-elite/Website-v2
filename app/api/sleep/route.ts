...
export async function POST(request:Request){
  ...
  const today=new Date().toISOString().slice(0,10);const duration=clamp(body.durationHours,0,24);const quality=Math.round(clamp(body.quality,1,10));const now=new Date().toISOString()
