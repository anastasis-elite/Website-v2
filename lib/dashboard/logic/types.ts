...
export type HydrationResult = { consumed: number; target: number; remaining: number; percent: number; status: 'needs_input' | 'low' | 'building' | 'ready'; paceState: 'ahead_of_pace' | 'on_pace' | 'slightly_behind' | 'behind' | 'insufficient_data'; expectedPercent: number; dayElapsedPercent: number; prompt: string; recoverySupportNote: string }
...
export type StreakRequirementDestination = { href: string; ariaLabel: string }
export type StreakRequirements = { date: string; programTier: ProgramTier; capacityStatus: CapacityStatus; missedDayCount: number; requiredItems: Record<StreakItem, boolean>; completedItems: Record<StreakItem, boolean>; itemDestinations: Partial<Record<StreakItem, StreakRequirementDestination>>; completionScore: number; streakEligible: boolean; resetMessage: string | null }
