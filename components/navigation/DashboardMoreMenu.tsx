...
export default function DashboardMoreMenu({program}:{program:'ember'|'ignite'|'phoenix'}){
  ...
  return <div ref={ref} className="dashboard-more-menu"><button type="button" className="dashboard-more-trigger client-dashboard-nav__item client-dashboard-nav__item--more" data-nav-item="more" aria-expanded={open} aria-haspopup="dialog" onClick={()=>setOpen((value)=>!value)}><span className="client-dashboard-nav__icon" aria-hidden="true">•••</span><small>More</small></button>{open?<div className="dashboard-more-backdrop" onClick={()=>setOpen(false)}><div className="dashboard-more-sheet" role="dialog" aria-modal="true" aria-label="More options" onClick={(event)=>event.stopPropagation()}><div className="dashboard-more-heading"><div><small>{program}</small><h2>More</h2></div><button type="button" autoFocus onClick={()=>setOpen(false)} aria-label="Close more menu">×</button></div><nav>{items.map(([label,href])=>href.startsWith('mailto:')?<a key={label} href={href} onClick={()=>setOpen(false)}>{label}<span>›</span></a>:<Link key={label} href={href} onClick={()=>setOpen(false)}>{label}<span>›</span></Link>)}</nav><button type="button" className="dashboard-more-logout" disabled={loggingOut} onClick={logout}>{loggingOut?'Signing Out…':'Log Out'}</button></div></div>:null}</div>
}
