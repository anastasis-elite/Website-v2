  ...
  return (
    <main className="account-dashboard" style={{ '--account-intensity': Math.max(.25, flame.score / 100) } as CSSProperties}>
      <div className="account-shell">
        ...
        <section className="account-panel account-quick-panel"><div className="account-section-heading"><h2>Quick actions</h2><span>Manage your plan and progress</span></div><div className="account-quick-grid">{quickActions.map((action) => { const href = 'route' in action ? dashboardHref : action.href; return <Link href={href} key={action.title} className="account-quick-action"><span aria-hidden="true">{action.icon}</span><strong>{action.title}</strong><small>{action.detail}</small><b aria-hidden="true">→</b></Link> })}</div></section>
