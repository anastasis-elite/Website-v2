...
export default function SafetyEscalationNotice({ flags, embedded=false }: { flags: SafetyFlag[]; embedded?:boolean }) {
  const emergency = flags.some((flag) => flag.urgency === 'emergency')
  const content=<div style={{ ...styles.containerStyle, maxWidth: '780px' }}>
  ...
  return embedded?<section className="aos-safety-embedded">{content}</section>:<main style={styles.pageStyle}>{content}</main>
}
