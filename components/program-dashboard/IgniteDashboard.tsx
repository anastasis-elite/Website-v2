...
function momentum(score: number) {
  if (score === 100) {
    return 'Day complete. Let the work count.'
  }

  if (score >= 75) {
    return 'Strong momentum. Close the final loops.'
  }

  if (score >= 50) {
    return 'Keep the momentum going. Your next choice matters.'
  }

  if (score >= 25) {
    return 'The flame is building. Complete the next clear action.'
  }
...
function Sparkline({
  ...
  if (points.length < 2) {
    return (
      <span className="ignite-no-trend">
        Not enough data
      </span>
    )
  }
  ...
  return (
    <svg
      className="ignite-sparkline"
      viewBox="0 0 100 38"
      role="img"
      aria-label={`${trend.label} seven-day trend`}
    >
...
export default function IgniteDashboard({
  ...
  async function loadFoodSuggestions() {
    try {
      ...
      const response = await fetch('/api/quick-add-suggestions')
      const payload = await response.json().catch(() => null)

      if (!response.ok) {
        throw new Error(
          payload?.error ||
            'Quick food suggestions could not be loaded.'
        )
      }

      setFoodSuggestions(payload?.suggestions || [])
    } catch (error) {
      setFoodError(
  ...
  async function toggleFoodPopup() {
    const nextOpen = !foodOpen

    setFoodOpen(nextOpen)
    setFoodError('')

    if (nextOpen && foodSuggestions.length === 0) {
      await loadFoodSuggestions()
    }
  }
    ...
    >
      <div className="ignite-shell">
        ...
        <section className="ignite-top-grid">
          <article className="ignite-panel ignite-daily-progress">
            ...
            {foodOpen ? (
              ...
              >
                ...
                >
                  <div>
                    ...
                    <p
                      style={{
                        margin: 0,
                        color: 'rgba(215,199,182,0.7)',
                        fontSize: '0.86rem',
                        lineHeight: 1.5,
                      }}
                    >
                      Foods you frequently log during this time of day.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: 'rgba(181,110,67,0.06)',
                      color: '#f5f0e8',
                      cursor: 'pointer',
                      fontFamily: 'inherit',
                    }}
                ...
                {foodLoading ? (
                    ...
                    {foodSuggestions.map((suggestion, index) => (
                      <Link
                        ...
                        style={{
                          ...
                          background: 'rgba(181,110,67,0.05)',
                          color: '#f5f0e8',
                          textDecoration: 'none',
                        }}
                      >
                        ...
                        >
                          {Math.round(suggestion.calories || 0)} cal ·{' '}
                          {Math.round(suggestion.protein || 0)}g protein ·{' '}
                          {Math.round(suggestion.carbs || 0)}g carbs ·{' '}
                          {Math.round(suggestion.fats || 0)}g fat
                        </small>
                      </Link>
                    ))}
                  </div>
                  ...
                  >
                    No repeated foods qualify for quick suggestions yet.
                    Suggestions appear after the same foods are logged
                    consistently.
                  </p>
                )}
            ...
            <div className="ignite-momentum">
              <span className="ignite-momentum-flame">
                {flame.icon}
              </span>
        ...
        <section className="ignite-plan-insight-grid">
          ...
          <article
            id="ignite-insight"
            className="ignite-panel ignite-insight"
          >
            <div className="ignite-heading">
              <p className="ignite-label">
                Today&apos;s Insight
        ...
        <nav
          className="ignite-bottom-nav"
          aria-label="Ignite dashboard navigation"
        >
          <Link
