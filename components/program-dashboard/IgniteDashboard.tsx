    ...
    <main className="ignite-dashboard" style={{ '--ignite-intensity': flame.intensity } as CSSProperties}>
      <div className="ignite-shell">
        ...
        <section className="ignite-action-grid">
          <article className="ignite-panel ignite-action-card"><p className="ignite-label">↟ Today&apos;s Workout</p><h3>{workout.title}</h3><p>{workout.type}{workout.durationMinutes ? ` · ${workout.durationMinutes} min` : ''}</p><Link href="/dashboard/program/ignite/workout" className="ignite-button">{workout.executionComplete ? 'Workout Complete ✓' : 'View Workout'}</Link></article>
          <article className="ignite-panel ignite-action-card"><p className="ignite-label">✓ Assessments</p><h3>Daily Check-In</h3><p>{assessment.dailyCompleted ? 'Daily complete' : 'Daily check-in open'} · {assessment.monthlyDueCount} monthly due</p><div className="ignite-mini-ring">{assessment.completedPercent}%</div><Link href="/dashboard/assessment/monthly" className="ignite-button">Continue</Link></article>
          <article className="ignite-panel ignite-action-card"><p className="ignite-label">♥ Recovery Check</p><h3>{recovery.completed ? 'Check-in complete' : 'How are you feeling?'}</h3><p>Energy, stress, sleep, soreness and symptoms.</p><Link href="/dashboard/recovery" className="ignite-button">{recovery.completed ? 'View Recovery' : 'Log Now'}</Link></article>
